document.addEventListener('DOMContentLoaded',()=>{
    ...
    if(mt&&mn){
        mt.addEventListener('click',()=>{
            ...
            const s=mt.querySelectorAll('span'),a=mn.classList.contains('active');
        ...
        mn.querySelectorAll('a').forEach(a=>a.addEventListener('click',()=>{
            ...
            mt.querySelectorAll('span').forEach(s=>{s.style.transform='';s.style.opacity=''});
        }));
    }
    // Scroll Reveal
...
function initGrowthSlider(){
    ...
    const update=()=>{
        ...
        const lowPrice=d.volume*1695;
        const highPrice=d.volume*2119;
        if(el('growthMarketLow'))el('growthMarketLow').textContent='$'+Math.round(lowPrice).toLocaleString('de-DE');
        if(el('growthMarketHigh'))el('growthMarketHigh').textContent='$'+Math.round(highPrice).toLocaleString('de-DE');
        // Visual tree
...
function initConfigurator(){
    const s=document.getElementById('configSlider');
    if(!s)return;
    const ppt=250;
    const update=()=>{
        ...
        if(el('configPrice'))el('configPrice').textContent=(t*ppt).toLocaleString('de-DE')+' \u20AC';
        if(el('configArea'))el('configArea').textContent=(t*0.02).toFixed(1).replace('.',',');
        if(el('configCo2'))el('configCo2').textContent=(t*600).toLocaleString('de-DE');
        if(el('configVolume'))el('configVolume').textContent=(t*2).toLocaleString('de-DE');
        // Market value calculation
        const totalVol=t*2;
        const lowVal=totalVol*1695;
        const highVal=totalVol*2119;
        if(el('configMarketLow'))el('configMarketLow').textContent='$'+Math.round(lowVal).toLocaleString('de-DE');
        if(el('configMarketHigh'))el('configMarketHigh').textContent='$'+Math.round(highVal).toLocaleString('de-DE');
        // Forest visual
        const bars=document.querySelectorAll('.forest-bar');
        bars.forEach((b,i)=>{
            const active=i<Math.ceil((t/800)*bars.length);
            b.classList.toggle('active',active);
            b.style.height=active?(30+Math.random()*60)+'px':'8px';
        });
    };
    s.addEventListener('input',update);
    update();
